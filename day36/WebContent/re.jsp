...
<body>
	...
	<%
		if("GET".equals(request.getMethod())){
			...
			if(bean.getSeq()==0){
				//댓글
				String seq = "(select max(seq)+1 from bbs36 a where ref = "+bean.getRef()+")";
				sql = "insert into bbs36 values((select max(num)+1 from bbs36 a),'"+bean.getSub()+"', '"+bean.getContent()+"',"+bean.getRef()+","+seq+","+bean.getLvl()+"+1);";
			}else{
				//답글
				sql = "update bbs36 set seq=seq+1 where ref="+bean.getRef()+" and seq>"+bean.getSeq()+"; ";
				stmt.execute(sql);
				stmt.close();
				stmt=conn.createStatement();
				
				sql = "insert into bbs36 values((select max(num)+1 from bbs36 a),'"+bean.getSub()+"', '"+bean.getContent()+"',"+bean.getRef()+","+bean.getSeq()+"+1,"+bean.getLvl()+"+1);";
			}
