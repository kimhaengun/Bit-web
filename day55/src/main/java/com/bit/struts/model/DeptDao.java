...
public class DeptDao {
	...
	public List<DeptVo> findAll(){
		...
		MongoCollection<org.bson.Document> coll = db.getCollection(COLLECTIONNAME);
	...
	public void addOne(int deptno, String dname, String loc) {
		...
		try(
			...
			coll.insertOne(doc);
			//insertOne은 리턴 값이 void 이기 떄문에 수행 결과를 받아야함.
	...
	public DeptVo findOne(String id) {
		...
		try(
				...
				coll = client.getDatabase(DATABASENAME).getCollection(COLLECTIONNAME);
				Bson filter = Filters.eq("_id",_id);
				FindIterable<Document> ite = coll.find(filter);
				Document doc = ite.first();
				bean = new DeptVo(doc.getObjectId("_id"),
						doc.getInteger("deptno"),
						doc.getString("dname"),
						doc.getString("loc"));
				}
	...
	public long updateOne(String id, int deptno, String dname, String loc) {
		...
		try(
			...
			result = coll.updateOne(Filters.eq("_id",  new ObjectId(id)) , update );
