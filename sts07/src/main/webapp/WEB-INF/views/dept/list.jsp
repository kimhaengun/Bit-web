...
<div class="page-header">
	<h1>LIST PAGE <samll>DEPT LIST</samll></h1>
</div>
...
<div id="deleteform" class="modal fade" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
...
<script type="text/javascript">
	...
	$('.panel').click(function(){
		$.getJSON($(this).find('h3').text().substring(0,$(this).find('h3').text().indexOf('.')))
		 .done(function(data){
			 $('#editform').find('form').find('input').eq(0).val(data.deptno)
			 										  .end().eq(1).val(data.dname)
			 										  .end().eq(2).val(data.loc);
	...
	$('#editform').find('form').submit(function(){
		...
		$.ajax(this.deptno.value,{
			type:'put',
			data:JSON.stringify(param), //JSON.stringify --> JSON 객체는 서버가 읽을 수 있는 String 객체로 변환
	...
	$('#editform').find('form').find('.btn-danger').click(function(){
		$('#deleteform').modal('toggle').find('.btn-danger').click(function(){
